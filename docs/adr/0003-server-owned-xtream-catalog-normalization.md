...
9. General catalog identifiers and text are emitted only from an explicit allow-list of Provider fields. An allow-listed scalar is rejected as a direct credential echo only when its normalized value exactly equals the active Provider username or password; incidental substring overlap is not treated as credential evidence. Raw Provider credential fields such as `username` and `password` are never selected into HULK contracts. Provider-supplied image/metadata URLs remain a stricter boundary: they are restricted to HTTP/HTTPS, reject URL userinfo, reject any occurrence of the active Provider username/password, and reject known credential-like query parameter names. Unsafe URLs normalize to `null`. Phase 3 does not fetch or proxy image metadata.
...
- The browser receives stable HULK catalog JSON and never intentionally receives Provider credential fields, direct username/password echoes, reusable credential-bearing Provider URLs, raw Xtream objects, or playback URLs. Legitimate catalog IDs, names, and season keys are not destroyed merely because they incidentally contain the same short character sequence as a credential.
